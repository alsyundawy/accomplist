...
'''
==============================================================================================
 accomplist.py: v2.94-20240605-1 Copyright (C) 2018-2024 Chris Buijs <cbuijs@chrisbuijs.com>
==============================================================================================
    ...
    if prefix:
        ...
        if ipregex4.search(eip):
            if prefix in (8, 16, 24, 32):
                ...
                p = ((prefix + 8) / 8) * 8
                log_info('SPLIT: ' + eip + '/' + str(prefix) + ' --SPLIT--> /' + str(p))
                if p > 7:
                    for subnet in list(netaddr.IPNetwork(eip + '/' + str(prefix)).subnet(p)):
                        revip.append('.'.join(str(subnet).split('.')[0:p / 8][::-1]) + '.in-addr.arpa')
            ...
            if prefix in (4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92, 96, 100, 104, 108, 112, 116, 120, 124, 128):
                ...
                if p > 3:
                    for subnet in list(netaddr.IPNetwork(eip + '/' + str(prefix)).subnet(p)):
                        revip.append('.'.join(filter(None, regex.split('(.)', regex.sub(':', '', str(subnet)))))[0:(p / 4) * 2][::-1].strip('.') + '.ip6.arpa')
...
def expand_ip(ip):
    ...
    if debug >= 2: log_info('IPV6-EXPANDER: {0} -> {1}'.format(ip, new_ip))
            ...
            for line in f:
                ...
                if not (entry.startswith("#")) and not (len(entry) == 0):
                    ...
                    if len(element) > 2:
                        ...
                        if bw.endswith('!'):
                            bw = bw.rstrip('!')
                            if (debug > 1): log_info('The ' + bw.strip('!*^(><') + 'list \"' + listname + '\" will be FORCED!')
                            forced = True

                        getasn = False
                        if bw.endswith('*'):
                            bw = bw.rstrip('*')
                            if (debug > 1): log_info('Retrieving ASNs for ' + bw.strip('!*^(><') + 'list \"' + listname + '\"')
                            getasn = True

                        topcheck = False
                        if bw.endswith('^'):
                            bw = bw.rstrip('^')
                            if (debug > 1): log_info('Only use TOP domain of ' + bw.strip('!*^(><') + 'list \"' + listname + '\"')
                            topcheck = True

                        stripwww = True
                        if bw.endswith('('):
                            bw = bw.rstrip('(')
                            if (debug > 1): log_info('Do not strip WWW labels from ' + bw.strip('!*^(><') + 'list \"' + listname + '\"')
                            stripwww = False
                        ...
                        if bw.endswith('>'):
                            bw = bw.rstrip('>')
                            if (debug > 1): log_info('Accept any TLD ' + bw.strip('!*^(><') + 'list \"' + listname + '\"')
                            anytld = True

                        if bw.endswith('<'):
                            bw = bw.rstrip('<')
                            if (debug > 1): log_info('Reverse Domains will be generated for ' + bw.strip('!*^(><') + 'list \"' + listname + '\"')
                            revdom = True
